...
def train(loader, net, criterion, optimizer, device, debug_steps=100,
    ...
    for i, data in enumerate(loader):
        ...
        if i and i % debug_steps == 0:
            ...
            ########################
            writer.add_scalar('train/Average_Loss', avg_loss, global_step)
            writer.add_scalar('train/Average_Regression_Loss', avg_reg_loss,
                              global_step)
            writer.add_scalar('train/Average_Classification_Loss',
                              avg_clf_loss, global_step)
            global_step += 1
            ########################
...
if __name__ == '__main__':
    ...
    # 可视化网络模型
    ##################################
    a_inputs = torch.randn(1, 3, 320, 240)
    with writer:
        writer.add_graph(net, (a_inputs, ))
    ##################################
